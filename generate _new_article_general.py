...
CHANGES_AGGRESSIVE = f"""{description_of_fact} You need to choose totally different facts for the values of \"{name_of_fact}\". 
...
def generate_aggressive_prompts(topics):
    ...
    for topic in topics:
        ...
        common_examples = topics["Common examples"]
...
def transform_topics_to_fact_extraction_prompt(dataset, general_information, article):
    # Create a dictionary to store the transformed data
    transformed_dict = {}
    
    # Iterate over the dataset and fill the transformed dictionary
    for item in dataset:
        ## Extract the name and description
        name_of_fact = item.get('Name of fact', 'Unknown Fact')
        description = item.get('Description of fact', 'Description not available.')
        common_examples = item.get('Common examples', 'No common examples provided.')
...
def extract_and_change_articles(true_articles, name_of_the_model, size_of_sample=5, change_of_article = "paraphrase_abdul", print_comments = False):
    """
    Summary
...
def extract_and_change_articles_with_labeling_added(true_articles, name_of_the_model, size_of_sample=5, change_of_article = "paraphrase_abdul", print_comments = False):
    """
    Summary
