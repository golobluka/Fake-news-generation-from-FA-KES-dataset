...
GENERAL_CHANGES_AGGRESSIVE = """{description_of_fact} You need to choose totally different meaning of fact about \"{name_of_fact}\". 
...
PROMPT_FOR_CORRECTING_MISTAKES_WHEN_CHANGING_FACTS = """You are trying to change the fact relating to {name_of_fact} to have the following information: {new_fact}. Read the following article:

BEGINNING_OF_THE_ARTICLE
{article_in_process_of_being_changed}
END_OF_THE_ARTICLE

Your task is to change the fact relating to {name_of_fact}. The original fact was {original_fact}. This original fact was supposed to be changed to {new_fact}. New article should begin with the phrase: BEGINNING_OF_THE_ARTICLE and end with the phrase END_OF_THE_ARTICLE."""
...
GENERAL_CHANGE_ONE_INFORMATION = {
    ...
    "paraphrase_aggressive": """You have an article. {general_information}:
...
def change_articles_with_fact_already_generated(articles, name_of_the_model, size_of_sample=5, change_of_article = "paraphrase_aggressive", print_comments = False, testing = False):
    """
    Summary
    ...
    for index in indices:
        article_content = list(articles['article_content'])[index]
        json_dict = list(articles['json_dict'])[index]
        ...
        prompt_variables = {
            ...
            'Name_of_fact': topic_to_change[0]["Name of fact"],
            'changing_orders': topic_to_change[1]["Changing orders"],
            'general_information': GENERAL_INFORMATION,
        }
        if testing:
            generated = testing_dictionary['article_generation']
        ...
        if not changed_article or not changed_json:
            ...
            prompt_variables = {
                ...
                'Name_of_fact': topic_to_change[1]["Name of fact"],
                'changing_orders': topic_to_change[1]["Changing orders"],
                'general_information': GENERAL_INFORMATION,
            }
            if testing:
    ...
    Args:
        articles: DataFrame containing the articles and facts.
    ...
    for index in indices:
        article_content = list(articles['article_content'])[index]
        json_dict = list(articles['article_content'])[index]
        ...
        response = ollama.chat(model=name_of_the_model, messages=[
            {
                ...
                'content': change_one_information[change_of_article].format(
                    article=article_content,
                    facts =json.dumps(json_dict, indent=4, ensure_ascii=False),
                    Name_of_fact=topic_to_change[0][0],
                    changing_orders=topic_to_change[0][1]
                ),
                'temperature': 0.2,
            }
        ])
            ...
            response = ollama.chat(model=name_of_the_model, messages=[
                {
                    ...
                    'content': change_one_information[change_of_article].format(
                        article=changed_article,
                        facts =json.dumps(changed_json, indent=4, ensure_ascii=False),
                        Name_of_fact=topic_to_change[1][0],
                        changing_orders=topic_to_change[1][1]
                    ),
                    'temperature': 0.2,
                }
            ])
            ...
            for json_dict in json_list:
                ...
                for i in range(number_of_changed_facts):
                    ...
                    while not quality_of_generation and number_of_trials <= 3:
                        ...
                        article_in_process_of_being_changed = extract_last_article(generated)
                        if not article_in_process_of_being_changed or not json_in_process_of_being_changed:
                            ...
                            extracted = {
                                ...
                                "true_json_file": json_in_process_of_being_changed,
                                "topic": topic_names[i]
                            }
                            ...
                            if labeling_result['Labelled'] == True:
